...
public class ZXMahJongRule {
	...
	public static List<List<Integer>> huLaizi(List<Integer> list, int laizi,Map<Integer,List<Integer>> show) {
		...
		tingTable.add(laizi);
		tingTable.addAll(ZXMahJongRule.quePai2(allPai, laizi, num));
	...
	public static void main(String[] args) {
		...
		for (Integer data : allPai) {
			...
			Integer result = chosePai(pais, data, 1);
...
//		int ccc=tingPai(conversionType(aaa)).size();
	...
	public static boolean fitHu(int[][] memberPai, Map<Integer,List<Integer>> show){
		boolean huPai=false;
		if (ZXMahJongRule.huPai(memberPai)) {
			huPai = true;
		} else {
			if (show == null || show.size() == 0) {
				if (ZXMahJongRule.duiduiHu(memberPai)){
					huPai = true;
	 ...
	 * @return 杠的数目
	 */
	public static int judegeWithGang(List<Integer> shouPaiList, Map<Integer, List<Integer>> map) {
		...
		int[][] conversionType = ZXMahJongRule.conversionType(shouPaiList);
		for (int i = 0; i < 3; i++) {
			for (int j = 1; j < 10; j++) {
				if (conversionType[i][j] == 4) {
					gang_num++;
				}
			}
		}
	...
	public static Integer randomMoPai(List<Integer> shouPaiList, List<Integer> tablePais){
		...
		int[][] shouPaiArr = ZXMahJongRule.conversionType(shouPaiList);
		// 手牌的数量
		...
		// 对hm进行排序返回
		Map<Integer, Integer> hm2 = ZXMahJongRule.sortMapByValue(hm);
	...
	public static List<Integer> moPaiOne(List<Integer> shouPaiList) {
		List<Integer> randomList = new ArrayList<Integer>();
		List<Integer> pai = shouPaiList;
		int[][] paiArr = ZXMahJongRule.conversionType(pai);
		// 缺牌list
		List<Integer> quePai = ZXMahJongRule.quePai((paiArr));

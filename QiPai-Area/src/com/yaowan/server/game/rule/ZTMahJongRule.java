...
public class ZTMahJongRule {
	...
	public static List<List<Integer>> huLaizi(List<Integer> list, int laizi,Map<Integer,List<Integer>> show) {
		...
		tingTable.add(laizi);
		tingTable.addAll(ZTMahJongRule.quePai2(allPai, laizi, num));
	...
	public static void main(String[] args) {
		...
		for (Integer data : allPai) {
			...
			Integer result = chosePai(pais, data, 11);
...
//		int ccc=tingPai(conversionType(aaa)).size();
	...
	public static boolean isQueYiMen(int[][] allPai,
			Map<Integer, List<Integer>> map, int tuopai) {
	 ...
	 * @return
	 */
	public static int chosePai(List<Integer> list, int start, int tuopai) {
		int[][] allPai = conversionType(list);
		// 只留一种花色
	...
	public static boolean fitHu(int[][] memberPai, Map<Integer,List<Integer>> show){
		boolean huPai=false;
		if (ZTMahJongRule.huPai(memberPai)) {
			huPai = true;
		} else {
	...
	public static boolean judgeFlush(List<Integer> list, int laiZi) {
		...
		for (Integer integer : list) {
			if (integer != laiZi) {
				fistChar = String.valueOf(integer).substring(0, 1);
				break;
			}
		}
		for (Integer integer : list) {
	 ...
	 * @return
	 */
	public static boolean judegeWithGang(Map<Integer, List<Integer>> map) {
	...
	public static List<Integer> moPaiOne(List<Integer> shouPaiList) {
		...
		// 缺牌list
		List<Integer> quePai = ZTMahJongRule.quePai((paiArr));

...
import com.yaowan.server.game.model.data.dao.ZXMajiangDataDao;
 ...
 */
@Component
public class ZXMajiangDataFunction extends FunctionAdapter {
	
	@Autowired
	private ZXMajiangDataDao majiangDataDao;
	
	/**
	 * 缓存
	 */
	private final ConcurrentHashMap<Long, ZXMajiangData> cacheMap = new ConcurrentHashMap<>();
	
	@Override
	public void handleOnNextDay() {
		int week = TimeUtil.getDayOfWeek();
		if(week==1){
			for(Map.Entry<Long, ZXMajiangData> entry:cacheMap.entrySet()){
				ZXMajiangData majiangData = entry.getValue();
    ...
    public ZXMajiangData getZXMajiangData(long rid) {
    	ZXMajiangData data = cacheMap.get(rid);
    	if(data == null){
    		data = majiangDataDao.findByKey(rid);
    		if(data == null){
    			data = new ZXMajiangData();
    			data.setRid(rid);
    ...
    public void updateMajiangData(long rid,int power,int maxWinWeek,List<Integer> listHuType,boolean firstWeek,boolean success) {
    	ZXMajiangData majiangData = getZXMajiangData(rid);
		...
		if(success){
			...
			for (Integer integer : listHuType) {
				...
				case ZhenXiongWinType.ZX_GANG_SHANG_HUA_VALUE://杠上花
				...
				case ZhenXiongWinType.ZX_QING_DA_DUI_ZI_VALUE://清大对子
					...
					majiangData.setBigPairTotal(majiangData.getBigPairTotal()+1);	
					break;
				case ZhenXiongWinType.ZX_QING_XIAO_DUI_VALUE://清小对
					//清一色
					majiangData.setSameCardsWeek(majiangData.getSameCardsWeek()+1);
					majiangData.setSameCardsTotal(majiangData.getSameCardsTotal()+1);
					//巧7对

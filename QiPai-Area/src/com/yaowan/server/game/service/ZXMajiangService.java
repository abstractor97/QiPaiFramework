...
public class ZXMajiangService{
	...
	@Autowired
	private ZXMajiangFunction majiangFunction;

	@Autowired
	GlobalVar globalVar;
	
	@Autowired
	FriendRoomFunction friendRoomFunction;
	...
	public void playerPrepare(Player player) {
		...
		MajiangZhenXiongRoomCsv majiangRoomCsv = majiangRoomCache.getConfig(game
				.getRoomType());
		boolean flag = false;
	...
	public void playHand(Player player, final GBaseMahJong.OptionsType optionsType,
		...
		LogUtil.info("玩家的座位:"+member.getRole().getSeat()+"玩家的操作:"+optionsType+"===玩家操作的牌:"+pai);
		if (optionsType != optionsType.DISCARD_TILE
				&& optionsType != optionsType.SET_QUE_TYPE) {

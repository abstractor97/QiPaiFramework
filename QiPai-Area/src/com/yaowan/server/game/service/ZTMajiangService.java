...
public class ZTMajiangService {
	...
	public void playerPrepare(Player player) {
		...
		if (isAllPrepare) {
			 ...
			 * 
			 * @Override public void doTask(ISingleData singleData) { // 全部已经准备
			 * 开始麻将 ZTMaJongTable table = (ZTMaJongTable) singleData;
	...
	public void playHand(Player player,
			final GBaseMahJong.OptionsType optionsType, final List<Integer> pai) {

		LogUtil.info(optionsType + "player" + player.getRole().getNick());
...
//		table.getCanOptions().remove(member.getRole().getSeat());
		/*
		...
		LogUtil.info("玩家的昵称:"+member.getRole().getRole().getNick()+"玩家的座位:"+member.getRole().getSeat()+"玩家的操作:"+optionsType+"===玩家操作的牌:"+pai);
		if (optionsType != optionsType.DISCARD_TILE) {
			table.getReceiveQueue().put(member.getRole().getSeat(), optionsType);
		}
		manager.executeTask(new SingleThreadTask(table) {
			...
			public void doTask(ISingleData singleData) {
				...
				LogUtil.debug("等待对列:" + table.getQueueWaitType());
				try {
					...
					case OptionsType.PASS_VALUE: {// 过
						...
						if (table.isManyOperate() || table.isQiangGangHu()) {
							majiangFunction.doManyHand(table);
						} else {
					...
					case OptionsType.PENG_VALUE: {// 碰
						...
						if (table.getQueueWaitType() == HandleType.MAJIANG_WAIT
								...
								|| table.getQueueWaitType() == HandleType.MAJIANG_FREE_PENG
					...
					case OptionsType.FREE_PENG_VALUE: {// 飞碰
						System.out
								.println("FREE_PENG_VALUE table.getQueueWaitType()"
										+ table.getQueueWaitType());
						if (table.getQueueWaitType() == HandleType.MAJIANG_WAIT
								|| table.getQueueWaitType() == HandleType.MAJIANG_HU
								|| table.getQueueWaitType() == HandleType.MAJIANG_FREE_PENG
								|| table.getQueueWaitType() == HandleType.MAJIANG_MANY_HU) {
							LogUtil.info("飞碰service.......");
							if (table.isManyOperate()) {
								majiangFunction.doManyHand(table);
							} else {
								majiangFunction.dealFreePeng(table, member);
							}
							majiangFunction.checkTingPai(table, member);
							// if (majiangFunction.checkOrder(table, member,
					...
					case OptionsType.FREE_EXPOSED_GANG_VALUE: {// 飞杠
						if (table.getQueueWaitType() == HandleType.MAJIANG_WAIT
								|| table.getQueueWaitType() == HandleType.MAJIANG_HU
								|| table.getQueueWaitType() == HandleType.MAJIANG_FREE_PENG
								|| table.getQueueWaitType() == HandleType.MAJIANG_MANY_HU) {
							if (table.isManyOperate()) {
								...
								majiangFunction.dealFreeGang(table, member);
							}
							majiangFunction.checkTingPai(table, member);
						}
						break;
					...
					case OptionsType.DARK_GANG_VALUE: {// 暗杠
						if (table.getQueueWaitType() == HandleType.MAJIANG_OUT_PAI
								|| table.getQueueWaitType() == HandleType.MAJIANG_HU
								|| table.getQueueWaitType() == HandleType.MAJIANG_WAIT) {
					...
					case OptionsType.DARK_FREE_GANG_VALUE: {// 飞暗杠
						if (table.getQueueWaitType() == HandleType.MAJIANG_OUT_PAI
								|| table.getQueueWaitType() == HandleType.MAJIANG_HU
								|| table.getQueueWaitType() == HandleType.MAJIANG_WAIT) {
							majiangFunction.dealFreeAnGang(table, member,
									pai.get(0));
							majiangFunction.checkTingPai(table, member);
						}
						break;
					}
					case OptionsType.TI_TUO_VALUE: {// 提坨
						if (table.getQueueWaitType() == HandleType.MAJIANG_OUT_PAI
								|| table.getQueueWaitType() == HandleType.MAJIANG_HU
								|| table.getQueueWaitType() == HandleType.MAJIANG_WAIT) {
							majiangFunction.dealTiTuo(table, member, pai.get(0));
							LogUtil.info("提坨操作的牌:" + pai.get(0));
							majiangFunction.checkTingPai(table, member);
						}
						break;
					}
					case OptionsType.EXTRA_GANG_VALUE: {// 补杠
						if (table.getQueueWaitType() == HandleType.MAJIANG_OUT_PAI
								|| table.getQueueWaitType() == HandleType.MAJIANG_HU
								|| table.getQueueWaitType() == HandleType.MAJIANG_WAIT) {
							table.setBuGangPai(pai.get(3));
							table.setLastPai(pai.get(0));
						majiangFunction.dealQiangGangHu(table, member);
						LogUtil.info("table是否为抢杠胡:"+table.isQiangGangHu());
						if (!table.isQiangGangHu()) {
							if (pai == null) {
								List<Integer> data = new ArrayList<Integer>();
								data.add(0);
								majiangFunction.dealExtraGang(table,
										member, data.get(0));
							} else {
								majiangFunction.dealExtraGang(table,
										member, pai.get(0));
					...
					case OptionsType.EXTRA_FREE_GANG_VALUE: {// 飞补杠
						if (table.getQueueWaitType() == HandleType.MAJIANG_OUT_PAI
								|| table.getQueueWaitType() == HandleType.MAJIANG_HU) {
							table.setBuGangPai(pai.get(3));
							table.setLastPai(pai.get(0));
							LogUtil.info("飞补杠的牌:"+pai.get(3));
							majiangFunction.dealQiangGangHu(table, member);
							LogUtil.info("是否为抢杠胡状态:"+table.isQiangGangHu());
							if (!table.isQiangGangHu()) {
								if (pai == null) {
									List<Integer> data = new ArrayList<Integer>();
									data.add(0);
									majiangFunction.dealExtraFreeGang(table,
											member, data.get(0));
								} else {
									majiangFunction.dealExtraFreeGang(table,
											member, pai.get(0));
					...
					case OptionsType.ANNOUNCE_WIN_VALUE: {// 胡牌
						if (table.getQueueWaitType() == HandleType.MAJIANG_HU
								|| table.getQueueWaitType() == HandleType.MAJIANG_GANG
								|| table.getQueueWaitType() == HandleType.MAJIANG_FREE_GANG
								|| table.getQueueWaitType() == HandleType.MAJIANG_PENG
								|| table.getQueueWaitType() == HandleType.MAJIANG_FREE_PENG
								|| table.getQueueWaitType() == HandleType.MAJIANG_MANY_HU) {
							LogUtil.info("玩家发送胡牌命令 进入胡牌流程....");
							if (table.isManyOperate()) {
								majiangFunction.doManyHand(table);
							}else {
	...
	//客户端刷新手牌
	public void refreshPai(Player player) {
		ZTMaJongTable table = majiangFunction.getTableByRole(player.getRole()
				.getRid());
		ZTMajiangRole role = majiangFunction.getRole(player.getRole().getRid());
		majiangFunction.refreshPai(table,role);
	}
}
